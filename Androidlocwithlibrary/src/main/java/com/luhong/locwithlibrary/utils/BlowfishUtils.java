...
public class BlowfishUtils implements IBlowfishCrypter {

    public static final int MAXKEYLENGTH = 56;
    public static final int BLOCKSIZE = 8;
    static final String DEFAULT_KEY = "yunqi_public_key_^@)";//yunqi_public_key_^@)
    ...
    private static class SBlock {
        ...
        public SBlock(int nLo, int nHi) {
            m_Lo = nLo;
            m_Hi = nHi;
        }

        public SBlock(SBlock b) {
            m_Lo = b.m_Lo;
            m_Hi = b.m_Hi;
        }
    }
     ...
     * @param bfkey - 密钥
     */
    public BlowfishUtils(String bfkey) {
        setKey(bfkey);
    }
     ...
     * @param bfkey - 密钥
     */
    public BlowfishUtils(byte[] bfkey) {
        setKey(bfkey);
    }
    ...
    private void setKey(byte[] bytes) {
        ...
        for (int nI = 0; nI < PBOX_ENTRIES; nI++) {
        ...
        for (int nI = 0; nI < SBOX_ENTRIES; nI++) {
        ...
        for (int nI = 0; nI < PBOX_ENTRIES; nI++) {
            for (int nJ = 0; nJ < 4; nJ++) {
                nBuild = nBuild << 8 | bytes[nKeyPos] & 0xff;
                if (++nKeyPos == nLen) {
                    nKeyPos = 0;
                }
            }
        ...
        SBlock b = new SBlock(0, 0);
        for (int nI = 0; nI < PBOX_ENTRIES; nI += 2) {
            b = encryptBlock(b);
            m_pbox[nI] = b.m_Hi;
            m_pbox[nI + 1] = b.m_Lo;
        }
        for (int nI = 0; nI < SBOX_ENTRIES; nI += 2) {
            b = encryptBlock(b);
            m_sbox1[nI] = b.m_Hi;
            m_sbox1[nI + 1] = b.m_Lo;
        }
    ...
    protected SBlock encryptBlock(SBlock b) {
        int nHi = b.m_Hi;
        int nLo = b.m_Lo;
        int pbox[] = m_pbox;
        int sbox1[] = m_sbox1;
        ...
        nLo ^= pbox[17];
        return new SBlock(nHi, nLo);
    }

    protected SBlock decryptBlock(SBlock b) {
        int nHi = b.m_Hi;
        int nLo = b.m_Lo;
        nHi ^= m_pbox[17];
    ...
    private byte[] encrypt(byte buffer[]) {
        ...
        for (int nI = 0; nI < nLen; nI += 8) {
            SBlock b = bytesToBlock(buffer, nI);
            b = encryptBlock(b);
            M0 = concat(M0, blockToBytes(b));
        }
        return M0;
    }
    ...
    private SBlock bytesToBlock(byte buffer[], int nPos) {
        SBlock b = new SBlock();
        b.m_Hi = (int) ((buffer[nPos] & 0xFF) << 24 | (buffer[nPos + 1] & 0xFF) << 16 | (buffer[nPos + 2] & 0xFF) << 8 | (buffer[nPos + 3] & 0xFF));
        b.m_Lo = (int) ((buffer[nPos + 4] & 0xFF) << 24 | (buffer[nPos + 5] & 0xFF) << 16 | (buffer[nPos + 6] & 0xFF) << 8 | (buffer[nPos + 7] & 0xFF));
        return b;
    }

    private byte[] blockToBytes(SBlock b) {
        byte[] buffer = new byte[8];
        buffer[0] = (byte) (b.m_Hi >>> 24 & 0xFF);
        buffer[1] = (byte) (b.m_Hi >>> 16 & 0xFF);
        buffer[2] = (byte) (b.m_Hi >>> 8 & 0xFF);
        buffer[3] = (byte) (b.m_Hi & 0xFF);
        buffer[4] = (byte) (b.m_Lo >>> 24 & 0xFF);
        buffer[5] = (byte) (b.m_Lo >>> 16 & 0xFF);
        buffer[6] = (byte) (b.m_Lo >>> 8 & 0xFF);
        buffer[7] = (byte) (b.m_Lo & 0xFF);
        return buffer;
    }
    ...
    public String decrypt(String buffer) {
        byte[] buf = decrypt(hex2byte(buffer));
        int nCount = 0;
        for (int i = buf.length - 1; i >= 0 && buf[i] == 0x00; i--, nCount++) ;
        byte[] b = new byte[buf.length - nCount];
    ...
    public static void main(String[] args) {
        BlowfishUtils bf = new BlowfishUtils();
        bf.setKey("yunqi_public_key_^@)");
        String encrypt = bf.encrypt("867967024285441").trim();//加密 867967024285545
        System.out.println("加密encrypt= " + encrypt);
        String decrypt = bf.decrypt("fd8aee979e18884fe4a6adc6953eefdd").trim();//解密 922926C3489601ABD12C112FE41E69AF
