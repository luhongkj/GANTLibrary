package com.luhong.locwithlibrary.imagepicker.activity;
...
import com.luhong.locwithlibrary.imagepicker.adapter.ImageFoldersAdapter;
import com.luhong.locwithlibrary.imagepicker.adapter.ImagePickerAdapter;
import com.luhong.locwithlibrary.imagepicker.data.MediaFile;
import com.luhong.locwithlibrary.imagepicker.data.MediaFolder;
import com.luhong.locwithlibrary.imagepicker.executors.CommonExecutor;
import com.luhong.locwithlibrary.imagepicker.listener.MediaLoadCallback;
import com.luhong.locwithlibrary.imagepicker.manager.ConfigManager;
import com.luhong.locwithlibrary.imagepicker.manager.SelectionManager;
import com.luhong.locwithlibrary.imagepicker.provider.ImagePickerProvider;
import com.luhong.locwithlibrary.imagepicker.task.ImageLoadTask;
import com.luhong.locwithlibrary.imagepicker.task.MediaLoadTask;
import com.luhong.locwithlibrary.imagepicker.utils.DataUtil;
import com.luhong.locwithlibrary.imagepicker.utils.MediaFileUtil;
import com.luhong.locwithlibrary.imagepicker.utils.PermissionUtil;
import com.luhong.locwithlibrary.imagepicker.utils.Utils;
import com.luhong.locwithlibrary.imagepicker.view.ImageFolderPopupWindow;
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (!mediaFolderList.isEmpty()) {
                        //默认加载全部照片    会不会影响性能
                        ArrayList<MediaFile> mediaFileList = mediaFolderList.get(0).getMediaFileList();
                        if (!TextUtils.isEmpty(ConfigManager.getInstance().getSingPaths())) {
                            ArrayList<MediaFile> mediaFileArrayList = new ArrayList<>();
                            for (MediaFile file : mediaFileList) {
                                if (file.getPath().contains(ConfigManager.getInstance().getSingPaths())) {
                                    mediaFileArrayList.add(file);
                                }
                            }
                            mMediaFileList.addAll(mediaFileArrayList);
                        } else {
                            ArrayList<MediaFile> mediaFileArrayListTow = new ArrayList<>();
                            for (MediaFile file : mediaFileList) {
                                if (!file.getPath().contains("Pictures")) {
                                    mediaFileArrayListTow.add(file);
                                }
                            }
                            mMediaFileList.addAll(mediaFileArrayListTow);
                        }
